...
const FinanceTransactionForm: React.FC<Props> = ({
  ...
  const handleSave = async () => {
    ...
    const tx: Transaction = {
      ...
      amount,
      date,
      realizedAt: isPaid ? date : undefined,
      accountId,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <input className="w-full text-xl font-bold p-3 bg-transparent border-b-2 border-gray-100 dark:border-slate-800 outline-none focus:border-indigo-500 transition-colors dark:text-white" placeholder="Ex: Pagamento Fornecedor" aria-label="Ex: Pagamento Fornecedor" value={description} onChange={e = /> setDescription(e.target.value)} aria-label="Descricao" />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Conta</label>
                        <select className="w-full p-4 rounded-xl border border-gray-200 dark:border-slate-700 dark:bg-slate-800 outline-none text-sm dark:text-white" value={accountId} onChange={e = aria-label="Selecionar"> setAccountId(e.target.value)} aria-label="Conta">
                    ...
                    {type !== 'TRANSFER' && (
                        <div>
                            ...
                            <select className="w-full p-4 rounded-xl border border-gray-200 dark:border-slate-700 dark:bg-slate-800 outline-none text-sm dark:text-white" value={categoryId} onChange={e = aria-label="Selecionar"> setCategoryId(e.target.value)} aria-label="Categoria">
                                <option value="">Selecione...</option>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col justify-end">
                        <label className="flex items-center gap-2 cursor-pointer p-4 rounded-xl border border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/30 dark:text-slate-100">
                            <input type="checkbox" className="w-5 h-5 rounded text-indigo-600" checked={isPaid} onChange={e = aria-label="Selecionar" /> setIsPaid(e.target.checked)} aria-label="Lancamento realizado" />
            ...
            <div className="space-y-6">
                ...
                <div className={`p-5 rounded-2xl border transition-all ${isRecurring ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/10' : 'border-gray-100 dark:border-slate-800'}`}>
                    <label className="flex items-center justify-between cursor-pointer mb-4">
                        ...
                        <input type="checkbox" className="w-6 h-6 rounded-lg text-blue-600" checked={isRecurring} onChange={e = aria-label="Selecionar" /> setIsRecurring(e.target.checked)} aria-label="Recorrente" />
                    </label>
                    
                    {isRecurring && (
                        <div className="space-y-3 animate-in fade-in">
                            <select className="w-full p-3 rounded-lg border dark:bg-slate-800 text-sm dark:text-slate-100" value={recurrenceRule} onChange={e = aria-label="Selecionar"> setRecurrenceRule(e.target.value)} aria-label="Regra de recorrencia">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input className="w-full p-4 rounded-xl border border-gray-200 dark:border-slate-700 dark:bg-slate-800 outline-none text-sm dark:text-white" placeholder="Ex: Marketing" aria-label="Ex: Marketing" value={costCenter} onChange={e = /> setCostCenter(e.target.value)} aria-label="Centro de custo" />
                    </div>
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input className="w-full pl-8 p-4 rounded-xl border border-gray-200 dark:border-slate-700 dark:bg-slate-800 outline-none text-xs dark:text-white" placeholder="fixo, variável..." aria-label="fixo, variável..." value={tags} onChange={e = /> setTags(e.target.value)} />
                        </div>
                    </div>
                </div>
            </div>
